...
`;
	var A = class extends HTMLElement {
		...
		render() {
			this.appendChild(_.content.cloneNode(!0)),
	...
	window.customElements.get("codapi-toolbar") ||
		customElements.define("codapi-toolbar", A);
	var T = {
			running: "Running...",
			failed: "\u2718 Failed",
			done: "\u2713 Done",
	...
	window.customElements.get("codapi-status") ||
		customElements.define("codapi-status", k);
	function V(n) {
		...
		return (
			...
			n.forEach((o) => {
				...
				Object.values(o).forEach((u) => {
					...
					(x.textContent = u), r.appendChild(x);
				}),
					s.appendChild(r);
			}),
			t.appendChild(e),
	...
	var G = {
			...
			[d.svg]: (n) => {
				...
				return t.querySelector("parsererror")
					? document.createTextNode(n.stdout)
					: t.documentElement;
			},
			[d.html]: (n) => {
				if (!n.stdout) return document.createTextNode(n.stderr);
				...
				return (
					Array.from(t.body.childNodes).forEach((s) =>
						e.appendChild(s)
					),
					e
				);
			...
			[d.dom]: (n) =>
				n.stdout ? n.stdout : document.createTextNode(n.stderr),
			[d.hidden]: (n) =>
				!n.stdout && n.stderr
					? document.createTextNode(n.stderr)
					: null,
		},
...
${t.stack}`
	...
	window.customElements.get("codapi-output") ||
		customElements.define("codapi-output", C);
	var p = { off: "off", basic: "basic", external: "external" },
		E = class extends EventTarget {
			...
			handleIndent(t) {
				t.key == "Tab" &&
					(t.preventDefault(),
					document.execCommand("insertHTML", !1, " ".repeat(4)));
			}
			handleHide(t) {
				t.key == "Escape" &&
					(t.preventDefault(), this.dispatchEvent(new Event("hide")));
			}
			handleExecute(t) {
				(!t.ctrlKey && !t.metaKey) ||
					(t.key !== "Enter" && t.key !== "NumpadEnter") ||
					(t.preventDefault(), this.executeFunc());
			}
			onPaste(t) {
				t.preventDefault();
				let e = (t.originalEvent || t).clipboardData.getData(
					"text/plain"
				);
...
`
	...
	var S = class extends HTMLElement {
		connectedCallback() {
			this.ready ||
				((a.settings.url = this.getAttribute("url")),
				(this.ready = !0));
		}
	...
	window.customElements.get("codapi-settings") ||
		customElements.define("codapi-settings", S);
...
`);
		return { method: i, url: o + r.join(""), headers: u, body: x };
	}
	async function nt(n) {
		let t = new Date(),
			e = await st(n),
...
`),
	...
	var F = { exec: rt };
	var O = { javascript: F.exec, fetch: N.exec };
	async function lt(n) {
		try {
			return ht(n)(n);
		} catch (t) {
	...
	function ht(n) {
		if (!(n.sandbox in O)) throw Error(`unknown sandbox: ${n.sandbox}`);
		if (n.command != "run")
...
`;
	var v = class extends HTMLElement {
		...
		render() {
			this.appendChild(q.content.cloneNode(!0));
			let t = this.findCodeElement();
			(this.snippet = new E(t, this.editor, this.execute.bind(this))),
			...
			this.hasAttribute("status-running") &&
				s.setAttribute("running", this.getAttribute("status-running")),
				this.hasAttribute("status-failed") &&
					s.setAttribute(
						"failed",
						this.getAttribute("status-failed")
					),
				this.hasAttribute("status-done") &&
					s.setAttribute("done", this.getAttribute("status-done")),
				...
				this.hasAttribute("output") &&
					(this._fallback = this.extractFallback(
		...
		extractFallback(t) {
			...
			return (
				...
				{ ok: !1, duration: 0, stdout: i, stderr: "" }
			);
		}
		findOutputElement(t) {
			if (!t || t == "@next")
	...
	function vt(n) {
		let t = new Set(),
			e = [],
			s = (o, r) => {
				if (!o) throw new Error(`#${r} dependency not found`);
				t.has(r) ||
					(t.add(r),
					o.dependsOn &&
						o.dependsOn.split(" ").forEach((u) => {
							s(document.getElementById(u), u);
						}),
					e.push(o.code));
			};
		s(n, n.id);
		let i = n.hasAttribute("output-tail") ? j.hr(n.syntax) : "";
		return i
			? e.join(`
${i}
`)
...
`);
	...
	window.customElements.get("codapi-snippet") ||
		customElements.define("codapi-snippet", v);
})();
